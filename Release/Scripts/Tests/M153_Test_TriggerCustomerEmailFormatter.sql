/*
	Tests for Triggers for CustomerEmailFormatting
	Module:			M153
	Authors:		Tim Bernhard
					Demian Flury
	Language:		T-SQL
	Database:		FitnessCourse

	Indication:		Please do not execute the whole script at once. Only execute the single tests!
*/

/*
	Test 1
	We want to test: Will the trigger work?
*/
UPDATE dbo.Customer
SET Email = 'UnForMattEd@eMaIl.com'
WHERE CustomerID = 1;

SELECT * FROM dbo.Customer WHERE CustomerID = 1;

/*
	Test 2
	We want to test: Will the trigger work with multiple rows in the Customer table?
*/
UPDATE dbo.Customer
SET Email = 'UnForMattEd@eMaIl.com'
WHERE CustomerID IN (1, 2, 3);

SELECT * FROM dbo.Customer WHERE CustomerID IN (1, 2, 3);

/*
	Test 3
	We want to test: Will the trigger work with a NULL value?
*/
UPDATE dbo.Customer
SET Email = NULL
WHERE CustomerID = 1;

SELECT * FROM dbo.Customer WHERE CustomerID = 1;